<?php

require_once "../requisitos.php";

verificar_logado();

$form = !empty($_GET["id"]);

if($form){
  $id = $_GET["id"];
  $pe = get_parceiro_especifico($id);

  $form_2 =  !empty($_GET["nome"]) && !empty($_GET["logo"]) && isset($_GET["tamanho"]);
  if($form_2){
  $nome = $_GET["nome"];  
  $logo = $_GET["logo"];
  $tamanho = $_GET["tamanho"];
    editar_parceiro($nome, $logo, $tamanho, $id);
    header("Location: parceiros.php");
  }
}

?>
  <main class="container-fluid">

    <div class="row">
      <div class="col-11 col-sm-10 m-auto mt-4 border border-4 border-secondary rounded-4 p-4">
        <h2>Editar - Menu Parceiros</h2>
        <br>
        <form action="">
          <input type="hidden" name="id" value="<?= $pe["id"] ?>">

          <input type="text" name="nome" placeholder="Nome" required value="<?= $pe["nome"] ?>">
          <br><br>
          <input type="text" name="logo" placeholder="logo" required value="<?= $pe["logo"] ?>">
          <br>
          <a href="../tinyfilemanager-master/tinyfilemanager.php" target="_blank" class="mt-1 btn btn-dark">Gestor de ficheiros</a>
          <br><br>

          <label for="tamanho">Tamanho:</label>
          <select name="tamanho" id="tamanho" required>
            <option value="1" <?= ($pe["tamanho"]) ? "selected" : "" ?>>Grande</option>
            <option value="0" <?= (!$pe["tamanho"]) ? "selected" : "" ?>>Pequeno</option>
          </select>
          <br><br>


          <input type="submit" class="btn btn-warning" value="Editar Destaque">
        </form>
      </div>
    </div>

  </main>